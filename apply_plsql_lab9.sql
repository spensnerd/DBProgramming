...
-- Call seeding libraries from within the following files.
...
-- Procedure for upload_item_description.
CREATE OR REPLACE
  PROCEDURE upload_item_description
  ( pv_partial_title VARCHAR2 ) IS

/* Translate a partial title 
   to a valid item ID */
  CURSOR get_item_id
...
-- Test the update_item_description.
EXECUTE upload_item_description('Harry Potter');


SELECT 'Faster Than Light' As UPLOAD_TEST from DUAL;
